...
let uEarthMap: WebGLUniformLocation;
...
// Lighting uniforms
let vAmbientDiffuseColor: WebGLUniformLocation;
let vSpecularColor: WebGLUniformLocation;
...
function generateSphere(subdiv: number) {
    ...
    for (let lat = 0; lat <= Math.PI; lat += step) {
        for (let lon = 0; lon + step <= 2 * Math.PI; lon += step) {
            ...
            let tangent1 = new vec4(0,1,0,0).cross(new vec4(
                Math.sin(lat)*Math.sin(lon),
                Math.cos(lat),
                Math.cos(lon)*Math.sin(lat),
                0.0
            ));
            let tangent2 = new vec4(0,1,0,0).cross(new vec4(
                Math.sin(lat+step)*Math.sin(lon+step),
                Math.cos(lat+step),
                Math.cos(lon+step)*Math.sin(lat+step),
                0.0
            ));
...
function update() {
    ...
    cloudRotateAngle += .1;
    if (cloudRotateAngle >= 360) cloudRotateAngle -= 360;
